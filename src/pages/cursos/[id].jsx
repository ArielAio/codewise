          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <aside className="lg:col-span-1 bg-[#001a33] rounded-lg p-6 shadow-lg border border-[#00FA9A] relative">
              ...
              {course.youtubeLinks && course.youtubeLinks.length > 0 ? (
                // Aumentando a altura do contêiner da lista de aulas
                <div className="space-y-4 h-[480px] overflow-y-auto overflow-x-hidden">
                  <ul>
                    {course.youtubeLinks.map((link, index) => (
                      <motion.li
                        key={index}
                        className={`bg-[#001a33] shadow-lg p-4 rounded-lg hover:bg-opacity-80 transition-colors cursor-pointer flex items-center justify-between ${selectedVideo === new URL(link.url).searchParams.get('v') ? 'border-2 border-[#00FA9A]' : ''
                          }`}
              ...
              {/* Barra de progresso posicionada no fundo */}
